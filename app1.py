...
st.title("🆕 Création manuelle — Routes validées automatiquement")
...
# --- Vérification automatique des routes existantes ---
invalid_routes = []
for r in st.session_state.routes:
    match = re.match(rf"^{prefix}(\d+)$", r["name"])
    if not match:
        invalid_routes.append(f"{r['name']} (préfixe incorrect)")
    else:
...
if invalid_routes:
    st.warning("⚠️ Routes incohérentes détectées : " + ", ".join(invalid_routes))
    st.info("💡 Modifie la plage ou corrige ces routes avant de générer la configuration.")

st.markdown("---")
st.subheader("🚗 Ajouter de nouvelles routes")
...
if st.session_state.routes:
    ...
    if st.button("🚀 Générer la configuration finale"):
        # Vérifier à nouveau avant génération
        invalid_routes = []
        for r in st.session_state.routes:
            match = re.match(rf"^{prefix}(\d+)$", r["name"])
            if not match:
                invalid_routes.append(f"{r['name']} (préfixe invalide)")
            else:
                num = int(match.group(1))
                if not (min_route <= num <= max_route):
                    invalid_routes.append(f"{r['name']} ({num} hors plage)")
        ...
        if invalid_routes:
            st.error("🚫 Des routes ne respectent pas la plage définie : " + ", ".join(invalid_routes))
            st.stop()
